    ...
    <section className="panel document-panel" id="documents" aria-labelledby="documents-heading">
      ...
      {!documents.length ? (
        ...
        <div className="table-scroll document-table-scroll">
          <table>
            <thead><tr><th className="check-cell"><span className="visually-hidden">Select</span></th><th>Document</th><th>Return</th><th>Client GSTIN</th><th>Period</th><th className="number-cell">Records</th><th>Review state</th><th>Uploaded</th><th className="action-cell">Action</th></tr></thead>
            <tbody>
              {documents.map((document) => {
                ...
                return (
                  <tr key={document.id} className={selected ? "row-selected" : ""}>
                    <td className="check-cell"><label className="checkbox"><input type="checkbox" checked={selected} onChange={() => onToggle(document.id)} aria-label={`Select ${document.originalName}`} /><span><Check size={13} /></span></label></td>
