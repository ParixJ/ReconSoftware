...
function portalSummaryRows(text) {
  ...
  const portalDefinitions = [
    ["4A", "taxableOutward", "B2B regular invoices", /4A\s*-\s*Taxable\s+outward\s+supplies\s+made\s+to\s+registered\s+persons[\s\S]{0,700}?Total\s+\S+\s+Invoice/i, /\b4B\s*-|\b4C\s*-|\b5\s*-/i, [0, 1, 2, 3, 4], 5],
    ["5", "taxableOutward", "B2C large", /5\s*-\s*Taxable\s+outward\s+inter-state\s+supplies\s+made\s+to\s+unregistered\s+persons[\s\S]{0,700}?Total\s+\S+\s+Invoice/i, /\b6[A-C]?\s*-/i, [0, 1, undefined, undefined, 2], 3],
    ["6A", "zeroRated", "Exports", /6A\s*-\s*(?:Taxable\s+outward\s+supplies\s+made\s+to\s+)?Exports[\s\S]{0,700}?Total\s+\S+\s+Invoice/i, /\b6B\s*-|\b6C\s*-|\b7\s*-/i, [0, 1, undefined, undefined, 2], 3],
    ["6B", "zeroRated", "SEZ supplies", /6B\s*-\s*(?:Taxable\s+outward\s+supplies\s+made\s+to\s+)?SEZ\s+supplies[\s\S]{0,700}?Total\s+\S+\s+Invoice/i, /\b6C\s*-|\b7\s*-/i, [0, 1, 2, 3, 4], 5],
    ["6C", "taxableOutward", "Deemed exports", /6C\s*-\s*Deemed\s+Exports[\s\S]{0,700}?Total\s+\S+\s+Invoice/i, /\b7\s*-/i, [0, 1, 2, 3, 4], 5],
    ["7", "taxableOutward", "B2C others", /7\s*-\s*Taxable\s+supplies[\s\S]{0,900}?Total\s+\S+\s+Net\s+Value/i, /\b8\s*-|\b9[A-B]?\s*-/i, [0, 1, 2, 3, 4], 5],
  ];
  for (const [section, category, description, start, end, indexes, minimumAmounts] of portalDefinitions) {
    const values = amountsAfter(text, start, end, 500);
    if (values?.length >= minimumAmounts) rows.push(moneyRow({ section, category, documentType: "gstr1", description, values, indexes }));
  }
