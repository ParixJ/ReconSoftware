...
const ANOMALY_ROOT_FIELDS = Object.freeze({
  ...
  GSTR1_PERIOD_NOT_FOUND: "returnPeriod",
  GSTR3B_PERIOD_NOT_FOUND: "returnPeriod",
  MULTIPLE_GSTR1_FOR_PERIOD: "returnPeriod",
  MULTIPLE_GSTR3B_FOR_PERIOD: "returnPeriod",
  MULTIPLE_GSTR2B_FOR_PERIOD: "returnPeriod",
  NO_RECORDS: "rows",
...
export function auditNormalized(normalized) {
  ...
  if (normalized.documentType === "unknown") {
    anomalies.push(anomaly(EXCEPTION_CODES.UNKNOWN_DOCUMENT_TYPE, "error", "The GST return type could not be identified.", "Open Modify mapping and choose the return type."));
  }
  if (!normalized.gstin) {
    anomalies.push(anomaly(EXCEPTION_CODES.MISSING_CLIENT_GSTIN, "error", "Client GSTIN was not found in the document.", "Open Modify mapping and enter the client GSTIN."));
  } else if (!new RegExp(`^${GSTIN_PATTERN.source.replace(/^\\b|\\b$/g, "")}$`, "i").test(normalized.gstin)) {
    anomalies.push(anomaly(EXCEPTION_CODES.INVALID_CLIENT_GSTIN, "error", `Client GSTIN ${normalized.gstin} is not in the expected 15-character format.`, "Verify the source or correct the GSTIN in Modify mapping."));
  }
  ...
  if (!normalized.returnPeriod && !hasBooksPeriods) {
    anomalies.push(anomaly(EXCEPTION_CODES.MISSING_RETURN_PERIOD, "error", "Return period was not found in the document.", "Open Modify mapping and enter the period as MMYYYY or MMYYYY-MMYYYY."));
  }
  if (!normalized.rows.length) {
    anomalies.push(anomaly(EXCEPTION_CODES.NO_RECORDS, "warning", "No structured records were extracted.", "Check whether the return is empty or map the source columns manually."));
  }
  ...
  normalized.rows.forEach((row, index) => {
    if (row.counterpartyGstin && !GSTIN_PATTERN.test(String(row.counterpartyGstin).toUpperCase())) {
      anomalies.push(anomaly(EXCEPTION_CODES.INVALID_COUNTERPARTY_GSTIN, "error", `Row ${index + 1} has an invalid counterparty GSTIN.`, "Verify the supplier/recipient GSTIN in the source.", index));
    }
    if (row.invoiceNumber && normalized.documentType !== "salesRegister") {
      ...
      if (invoiceKeys.has(key)) {
        anomalies.push(anomaly(EXCEPTION_CODES.DUPLICATE_INVOICE, "warning", `Invoice ${row.invoiceNumber} appears more than once for the same counterparty.`, "Review amendments and duplicate uploads before relying on totals.", index));
    ...
    if (!isReverseCharge && asNumber(row.invoiceValue) > 0 && componentTotal - asNumber(row.invoiceValue) > 1) {
      anomalies.push(anomaly(EXCEPTION_CODES.INVOICE_TOTAL_INCONSISTENT, "warning", `Invoice ${row.invoiceNumber || index + 1} is lower than taxable value plus tax.`, "Check mapped amount columns and rounding in the source.", index));
    }
  });
  return anomalies;
}
